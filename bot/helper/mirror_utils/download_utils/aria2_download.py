            ...
            if any([ZIP_UNZIP_LIMIT, LEECH_LIMIT, TORRENT_DIRECT_LIMIT, STORAGE_THRESHOLD]) and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
                ...
                if STORAGE_THRESHOLD:
                    ...
                    if not acpt:
                        ...
                        return await api.remove([download], force=True, files=True)
...
async def __onDownloadComplete(api, gid):
    ...
    if download.followed_by_ids:
        ...
        dl = await getDownloadByGid(new_gid)
        if dl :
            listener = await dl.listener()
        ...
        if dl:
            if hasattr(dl, 'listener') and dl.seeding:
                ...
                await dl.listener().onUploadError(f"Seeding stopped with Ratio: {dl.ratio()} and Time: {dl.seeding_time()}")
                await api.remove([download], force=True, files=True)
    else:
        LOGGER.info(f"onDownloadComplete: {download.name} - Gid: {gid}")
        dl = getDownloadByGid(gid)
        if dl:
            await asyncio.run(dl.listener().onDownloadComplete())
            await api.remove([download], force=True, files=True)
...
async def __onDownloadStopped(api, gid):
    ...
    dl = getDownloadByGid(gid)
    if dl :
        await dl.listener().onDownloadError('Dead torrent!')
...
async def start_listener():
    await aria2.listen_to_notifications(threaded=True,
